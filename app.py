...
DB_PATH = BASE / 'data' / 'career_bot.db'
...
CAREER_CLUSTERS = {
    'Engineering & Technology': {
        ...
        'studies': ['Computer Science', 'Software Engineering', 'Electrical Engineering', 'Mechanical Engineering', 'Cybersecurity'],
        'examples': ['Software engineer', 'Systems engineer', 'Product engineer', 'Cybersecurity analyst'],
        'activities': ['Build a small app', 'Try robotics or CAD', 'Take part in a coding or STEM challenge'],
    },
    'Research & Analysis': {
        'description': 'Evidence-based, investigative work that rewards curiosity, depth, and structured thinking.',
        'studies': ['Data Science', 'Economics', 'Mathematics', 'Physics', 'Politics/Policy', 'Biology'],
        'examples': ['Data analyst', 'Researcher', 'Economist', 'Policy analyst'],
        'activities': ['Run a mini research project', 'Analyse a dataset', 'Write a structured evidence brief'],
    },
    'Creative & Design': {
        'description': 'Idea generation, storytelling, design, and original expression.',
        'studies': ['Graphic Design', 'Product Design', 'Architecture', 'Marketing', 'Media', 'Writing'],
        'examples': ['Designer', 'Brand strategist', 'Content creator', 'Creative director'],
        'activities': ['Build a portfolio in Canva or Figma', 'Create a short video project', 'Write and publish something'],
    },
    'People, Health & Education': {
        'description': 'People-centered paths involving empathy, development, support, and communication.',
        'studies': ['Psychology', 'Education', 'Speech Therapy', 'Nursing', 'Medicine', 'Social Sciences'],
        'examples': ['Teacher', 'Psychologist', 'Coach', 'Healthcare professional'],
        'activities': ['Volunteer', 'Tutor someone younger', 'Interview a psychologist or teacher'],
    },
    'Business, Law & Leadership': {
        ...
        'activities': ['Pitch an idea', 'Join debate or Model UN', 'Run a small event or project'],
    },
    'Operations, Finance & Project Delivery': {
        ...
        'activities': ['Plan a project end-to-end', 'Build a simple budget model', 'Optimise a process in a spreadsheet'],
    },
}
...
QUESTIONS = [
    {
        ...
        'weights': {'Engineering & Technology': 3, 'Research & Analysis': 2, 'Operations, Finance & Project Delivery': 1},
        'type': 'ability'
    },
    {
        'key': 'numbers',
        'label': 'I like working with numbers, evidence, or patterns in data.',
        'weights': {'Research & Analysis': 3, 'Operations, Finance & Project Delivery': 2, 'Engineering & Technology': 2},
        'type': 'ability'
    },
    {
        'key': 'design',
    ...
    },
    {
        ...
        'weights': {'Research & Analysis': 3, 'Engineering & Technology': 1, 'People, Health & Education': 1},
        'type': 'energy'
    },
    {
        'key': 'stability',
    ...
    },
    {
        ...
        'weights': {'Research & Analysis': 2, 'Creative & Design': 1, 'Business, Law & Leadership': 1},
        'type': 'style'
    },
]
...
UNIVERSITY_DATA = {
    'UK': {
        'Engineering & Technology': ['Imperial College London', 'University of Manchester', 'University of Southampton', 'University of Bristol'],
        'Research & Analysis': ['University of Oxford', 'UCL', 'University of Warwick', 'London School of Economics'],
        'Creative & Design': ['University of the Arts London', 'Loughborough University', 'Kingston University London', 'University of Leeds'],
        'People, Health & Education': ['UCL', 'University of Edinburgh', 'University of Manchester', 'University of Glasgow'],
        'Business, Law & Leadership': ['LSE', 'University of Warwick', 'University of Bristol', 'King\'s College London'],
        'Operations, Finance & Project Delivery': ['University of Warwick', 'Bayes Business School', 'University of Bath', 'University of Leeds'],
    },
    'France': {
        ...
        'People, Health & Education': ['Université Paris Cité', 'Sorbonne Université', 'Université de Strasbourg', 'Université de Bordeaux'],
        'Business, Law & Leadership': ['HEC Paris', 'ESSEC', 'Sciences Po', 'Université Paris 1 Panthéon-Sorbonne'],
        'Operations, Finance & Project Delivery': ['ESCP', 'emlyon', 'Université Paris Dauphine-PSL', 'IAE Lyon'],
    },
    'Switzerland': {
        ...
        'People, Health & Education': ['University of Geneva', 'University of Lausanne', 'Bern University of Applied Sciences', 'University of Fribourg'],
        'Business, Law & Leadership': ['University of St.Gallen', 'HEC Lausanne', 'University of Geneva', 'University of Zurich'],
        'Operations, Finance & Project Delivery': ['University of St.Gallen', 'HEC Lausanne', 'ZHAW', 'SUPSI'],
    }
}
...
def load_assessments(profile_code):
    conn = sqlite3.connect(DB_PATH)
    df = pd.read_sql_query('SELECT * FROM assessments WHERE profile_code = ? ORDER BY created_at DESC', conn, params=(profile_code,))
    conn.close()
    return df
...
if page == 'Take an assessment':
    ...
    with col5:
        respondent_role = st.selectbox('You are completing this as', ['Self', 'Parent', 'Teacher', 'Friend', 'Coach/Mentor'])
    with col6:
    ...
    relation_weight = st.slider('Observer weight', min_value=0.4, max_value=1.2, value=float(default_weight), step=0.05,
    ...
    dream_day = st.text_area('Describe an ideal working day', placeholder='What would she be doing? Working with people, ideas, data, design, machines...')
    ...
    if st.button('Save assessment and show results', type='primary'):
        if not profile_name or not profile_code:
            ...
            score_df = pd.DataFrame({'Cluster': list(normalized_scores.keys()), 'Fit %': list(normalized_scores.values())}).sort_values('Fit %', ascending=False)
    ...
    if profile_code_lookup:
        ...
        if df.empty:
            ...
            view_df = df[['created_at', 'respondent_name', 'respondent_role', 'relation_weight', 'country_focus', 'favourite_subjects', 'least_favourite_subjects']].copy()
            ...
            if aggregated:
                ...
                score_df = pd.DataFrame({'Cluster': list(norm_avg.keys()), 'Fit %': list(norm_avg.values())}).sort_values('Fit %', ascending=False)
                st.bar_chart(score_df.set_index('Cluster'))
                ...
                st.download_button('Download profile data (CSV)', data=csv, file_name=f'{profile_code_lookup}_career_profile.csv', mime='text/csv')

...
def score_answers(answers):
    ...
    max_possible = {k: 0 for k in CAREER_CLUSTERS}

    for q in QUESTIONS:
        answer = int(answers.get(q["key"], 3))
        for cluster, weight in q["weights"].items():
            raw[cluster] += answer * weight
            max_possible[cluster] += 5 * weight

    normalized = {
        k: round((raw[k] / max_possible[k]) * 100, 1) if max_possible[k] else 0
        for k in CAREER_CLUSTERS
    }
...
def normalize_scores(raw_scores):
    ...
    normalized = {}
    for cluster, raw_value in raw_scores.items():
        denominator = max_possible[cluster] + context_bonus
        normalized[cluster] = round((max(raw_value, 0) / denominator) * 100, 1) if denominator else 0
